...
int_nu_dF.mx.LTRCforests <- function(T.pred, v, nu){
    ...
    inte = sapply(v, int_nu_dF.mx.LTRCforests.single, 
                  nu = nu, time=time, dFtz.mx=dFtz.mx)
...
int_nu_dF.mx.LTRCforests.single <- function(v, nu, time, dFtz.mx){
    n.est = nrow(dFtz.mx)
    vals = cbind(0, matrix(rep(nu(time), n.est), nrow = n.est, byrow = TRUE)*dFtz.mx)
...
# row - subjects; col - each v value
int_nu_dF.LTRCforests <- function(T.pred, v, nu){
    time = T.pred$survival.times
    Ftz.mx = 1 - t(T.pred$survival.probs)  # row - subject; col - different times points
    dFtz.mx = Ftz.mx - cbind(0, Ftz.mx[,-ncol(Ftz.mx)]) 
    n.est = nrow(Ftz.mx)
...
int_JG_m.LTRCforests <- function(T.pred.dF,jumps.Q, Fvz.mx, Gvz.mx, atrisk, tau, nu, trim){
    
    mvz.mx = int_nu_dF.mx.LTRCforests(T.pred.dF, jumps.Q, nu)
    dGvz.mx = Gvz.mx - cbind(0, Gvz.mx[,-ncol(Gvz.mx)])
