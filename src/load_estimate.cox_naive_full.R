...
load_estimate <- function(iteration_id, folder, nu, 
                          covariates.T, covariates.Q, 
                          alpha, n.boot){
  ...
  est.cox = est_DR.cox(dat, nu, covariates.T, covariates.Q)
  ...
  est_boot.cox = boot(dat, estimate.cox_boot, R = n.boot, nu = nu, 
...
estimate.cox_boot <- function(dat, id, nu, covariates.T, covariates.Q){
  dat = dat[id,]
  result.cox = est_DR.cox(dat, nu, covariates.T, covariates.Q)
...
## load the dataset and compute the naive estimator
load_estimate.naive_full <- function(iteration_id, folder, nu,alpha, n.boot){
  ...
  return(list(tab.naive = tab.naive,
              tab.full = tab.full,
              data.id = iteration_id))
}
