# Simulate data and then estimate using the DR, IPW.Q, IPW.T1, IPW.T2 with coxph for estimating the nuisance parameters, 
# plus the naive estimator and the full data estimator
# alpha: (1-alpha)*100% CI is computed 
c1.simu_estimate.cox_naive_full <- function(seed, nu, covariates.X, covariates.Q, 
                             trim.C, n.boot){
  ...
  beta = c(beta.Q = est.cox$beta, beta.T = est.cox$beta2)
  
  # SE and CI constructed using boostrap
  est_boot.cox = boot(dat, c1.estimate.cox_boot, R = n.boot, nu = nu, 
                      covariates.X = covariates.X, covariates.Q = covariates.Q, trim.C = trim.C)
  ...
  # SE and CI constructed using boostrap
  est_boot.naive = boot(dat, c1.estimator.naive_boot, R = n.boot, nu = nu, trim.C = trim.C)
...
c1.estimate.cox_boot <- function(dat, id, nu, covariates.X, covariates.Q, trim.C){
  dat = dat[id,]
  result.cox = c1.est_DR.cox(dat, nu, covariates.X, covariates.Q, trim.C)
  est.cox = c(DR = result.cox$est_DR, 
              IPQW = result.cox$est_IPQW, 
              IPSW = result.cox$est_IPSW, 
              IPSW2 = result.cox$est_IPSW2,
              beta.T = result.cox$beta.T,
              beta.Q = result.cox$beta.Q)
  
  return(est.cox)
}


c1.estimator.naive_boot <- function(dat, id, nu, trim.C){
  dat = dat[id,]
  if(sum(1-dat$delta) == 0){
    Sc = function(x){return(1)}
  }else{
