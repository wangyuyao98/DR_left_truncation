...
Gz <- function(t, Z, basehaz.Q2, beta.Q2){
    if(is.matrix(Z)){
...
integral_JG_F <- function(Q_original, time_original, Z, basehaz.Q2, basehaz.T, 
                          beta.Q2, beta.T, tau){
    Q2 = basehaz.Q2[,'time']   # increasing order
    Q = tau - Q2   # decreasing order
    Q = sort(Q)    # change to increasing order
    Fqz = Fz(Q, Z, basehaz.T, beta.T)
    Gqz = Gz(Q, Z, basehaz.Q2, beta.Q2)
...
# Q_orignal, time_original: the Q, T in the sample
integral_JG_m <- function(Q_original, time_original, Z, basehaz.Q2, basehaz.T,beta.Q2, beta.T, tau, nu){
    Q2 = basehaz.Q2[,'time']   # increasing order
    Q = tau - Q2   # decreasing order
    Q = sort(Q)    # change to increasing order
    mqz = integral_F(Q, nu, Z, basehaz.T, beta.T)
    Fqz = Fz(Q, Z, basehaz.T, beta.T)
    Gqz = Gz(Q, Z, basehaz.Q2, beta.Q2)
    dG = Gqz - cbind(0, Gqz[,-ncol(Gqz)])
    n = length(Q_original)
